...
namespace LAB02
{
    ...
    /// Ф.И.О - Фронская Полина Петровна
    /// Вариант - 4
    /// </summary>
    public class Solver
    {
        ...
        public static string ConvertSeconds(int seconds)
        {
            ...
            return $"{seconds} секунд = {hours} ч {minutes} мин {remainingSeconds} сек";
        }
        ...
        {
            // Используем оператор switch для определения описания оценки
            switch (grade)
            {
                case 1:
                    return $"Оценка {grade} - Плохо";
                case 2:
                    return $"Оценка {grade} - Неудовлетворительно";
                case 3:
                    return $"Оценка {grade} - Удовлетворительно";
                case 4:
                    return $"Оценка {grade} - Хорошо";
                case 5:
                    return $"Оценка {grade} - Отлично";
                default:
        ...
        public static string MaxOdd(int a, int b)
        {
            ...
            // Находим максимальное нечётное число, начиная с конца диапазона
            for (int i = end; i >= start; i--)
            {
                if (i % 2 != 0) // если число нечётное
                {
                    return $"Максимальное нечётное число в диапазоне [{start}, {end}] = {i}";
                }
            }

            // Если нечётных чисел нет
        ...
        public static string SumEvenInMatrix(int[,] matrix)
        {
            if (matrix == null)
            {
                return "Сумма четных элементов = 0";
            }
            ...
            return $"Сумма четных элементов = {sum}";  // ← ЭТОЙ СТРОКИ НЕ ХВАТАЛО!
        }
